...
- **`src/biomes.js`** — the `BIOMES` config table (currently twelve entries) plus `WILDFLOWER_PALETTES` / `GRASS_DENSITY` / `FLOWER_DENSITY` / `PEBBLE_DENSITY` overrides. Each biome fully specifies palette, fog, accent/sun colors, allowed flora kinds + count, particle type, creature color palette + count range, plus optional dusk/night palette deltas. Optional flags (`water`, `cloudlike`, `glowFlowers`, `glowEyes`, `fuzzy`, `creatureKind`) gate cross-cutting behavior — see "Biome-flag pattern" below. Adding visual variety usually means editing this table, not the builders.
...
- **`src/sky.js`** — backdrop primitives: gradient sky dome, two-layer wobbled mountain ring, drifting cloud sprites, twinkling starfield, and aurora curtains for cold/dreamy biomes. `updateSkyColors` re-tints the dome and mountains from the day/night palette; star/aurora opacity is driven by the night factor each frame. Per-biome cloud counts and aurora opt-ins live in the `CLOUD_COUNT` / `AURORA_BIOMES` / `AURORA_TINTS` tables in `biomes.js`.
...
- **`src/postfx.js`** — `initPostFX(renderer, scene, camera)` lazily builds an `EffectComposer` chain (RenderPass → UnrealBloomPass → custom tilt-shift ShaderPass → OutputPass). Bloom threshold is 0.85 so only emissive surfaces glow. Tilt-shift `uFocus` is updated each frame in `main.js` by projecting `(0, 1.5, 0)` through the camera so the sharp band follows the island during orbit. Returns a stub under `LOWFX`. Skipped entirely in inspect mode (the composer's tone mapping would crush the studio backdrop to black).
- **`src/fur.js`** — `applyShellFur(body, biome, opts)` adds N shells (8 / LOWFX 4) as children of a target mesh, sharing its geometry. Each shell uses a cloned `ShaderMaterial` differing only by `uShellLayer`. Vertex shader displaces along normal; fragment shader uses a 3D point hash on object-space cells to discard non-hair pixels. `sharedFurUniforms.uLightDir` is updated once per frame in `main.js`. Returns `null` under LOWFX. Called from `makeCreature` (walkers in fuzzy biomes) and `makeCaterpillar` (caterpillars but not snails in fuzzy biomes). Hidden on sleepers via the wake-cycle paths in `stepCreature`.
...
- **`src/inspect.js`** — `?inspect=1` URL gate. Replaces normal world-gen with a single specimen on a neutral studio backdrop (gradient dome + turntable disc + hemisphere/key/rim lights). Keyboard cycles `biome` (`[`/`]`), `variant` (`,`/`.`), reroll seed (`r`), pause (Space), and frame-step bidirectionally (`←`/`→`, also rewinds integrated `c.bob` / `c.age`). URL params (`biome`, `variant`, `seed`, `paused`) are parsed at boot and written back via `history.replaceState` on every state change so the address bar always reflects the exact view.
