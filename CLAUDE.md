...
- **`src/grass.js`** — `makeGrassField` (instanced grass-blade rendering with per-biome density / baldness). Re-exported from `environment.js` for backward compatibility, so callers can keep importing it from there.
